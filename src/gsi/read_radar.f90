...
!  Bigger loop over first level 2.5 data, and then level3 data
  ...
  ! do loop=1,2
     ...
     write(6,*)'READ_RADAR: L2RW radar radial wind obs error (user-definedin namelist)=',thiserr_rw,'(m/s).'
...
!     call readmg(lnbufr,subset,idate,iret)
!     if(iret/=0) then
!       write(6,*)'(mype=',mype,') L2 radar file is not in BUFR format! '
!       call closbf(lnbufr)
!       go to 1001
!     end if
             ...
             if(regional)then
                 ...
                 if (outside) then
                     if ( radartable_inside(irdr_ptr) > 0 ) then
                         write(6,*)'*** WARNING --> READ_RADAR: (columntilt) radar:',this_staid,' is found both inside and outside. Wrong! <-- WARNING ***'
                         call stop2(999)
                     end if
...
!          Go through the data levels
             ...
             loop_lvl : do k=1,levs
                 ...
                 nread=nread+1
                 ntb=ntb+1
!                 t4dvo=real(minobs+radar_obs_columntilt(1,k)-iwinbgn,r_kind)*r60inv
...
!                     timeo=t4dv
!                 else
!                     timeo=(real(minobs-mincy,r_kind)+real(radar_obs_columntilt(1,k),r_kind))*r60inv
...
!              obs box (???? not used for L2RWBUFR data)
!                 iaaa=azm/(r360/(r8*irrr))
!                 iaaa=mod(iaaa,8*irrr)
!                 if(iaaa<0) iaaa=iaaa+8*irrr
!                 iaaa=iaaa+1
...
!                      So good is actually same as good0
                 ...
                 if(.not.good0) then
                     ...
                     cycle loop_lvl
!                else
!                     Note: No further QC Check against vad wind quality mark for L2RWBUFR data
                 end if
